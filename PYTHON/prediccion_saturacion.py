...
ax.plot(df['fecha'], y, color='#81c784', linewidth=2, label='Histórico')
ax.plot(future_dates, future_pred, color='#ffd54f', linewidth=2,
        linestyle='--', label='Predicción')
ax.fill_between(future_dates, future_pred * 0.92, future_pred * 1.08,
                color='#ffd54f', alpha=0.15)

ax.set_title('Predicción de Nivel de Saturación (%)', color='white',
             fontsize=13, pad=12)
